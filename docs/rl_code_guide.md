...
| `thresholds.aisle_ramp` | 0.5 | w3 활성 선형 램프 폭 (5차 신규) — d_side 1.5→1.0m 동안 0→1. 입구 진입을 여전히 망설임 → ↑(더 완만, 0.8 후보). sac_2 실측: "망설이다 한 번에 빠른 진입"은 입구 페널티 관문(w3 램프+측면 β_s)+초기 충돌 Q-절벽에 대한 거의 최적 대응 — 통과 비용이 체류 시간에 비례하므로. 잔존 시 ↑와 `zeta`↓를 다음 런에서 함께 검토. 커리큘럼 속도 상향도 자연 교정 효과 |
